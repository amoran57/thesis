...
names(rate3month_mbd) <- c("t", "tmin1", "tmin2","tmin3","tmin4","tmin5","tmin6","tmin7","tmin8","tmin9","tmin10","tmin11")
rownames(rate3month_mbd) <- seq(1:nrow(rate3month_mbd))

#get formula call
call <- as.character(seq(1:(length(rate3month_mbd)- 1)))
...
monthly_dates <- seq(as.Date("1985/1/1"), as.Date("1995/1/1"), "month")
lag_order <- 12
...
for (k in 1:length(monthly_dates)) {
  ...
  X_test <- rate3month_mbd[nrow(rate3month_mbd), ]
  X_test$trend <- nrow(rate3month_mbd)
  rate3month_mbd <- rate3month_mbd[-nrow(rate3month_mbd),]
  
  #fit the forest
  timestamp()
  tic(paste0("Bayesian forest iteration ", as.character(k), " complete"))
  bayes <- bayes_reg_parallel_rf(formula, sample_data = sample_data, data = rate3month_mbd, penalties = penalties)
  toc()
...
forest_forecast_ts <- ts(forecasts_rf, start = c(1985, 1), frequency = 12)
...
pred_ar1 <- ts(pred_arima, start = c(1985,1), frequency = 12)
...
#Export ----------------------------------
write_rds(forest_forecast_ts, paste0(export,"other_cases/rate3month/base_forecast.rds"))
write_rds(pred_ar1, paste0(export, "other_cases/rate3month/ar1_forecast.rds"))