...
names(unemp_mbd) <- c("t", "tmin1", "tmin2","tmin3","tmin4","tmin5","tmin6","tmin7","tmin8","tmin9","tmin10","tmin11")
rownames(unemp_mbd) <- seq(1:nrow(unemp_mbd))

#get formula call
call <- as.character(seq(1:(length(unemp_mbd)- 1)))
...
monthly_dates <- seq(as.Date("1990/1/1"), as.Date("2000/1/1"), "month")
lag_order <- 12
...
for (k in 1:length(monthly_dates)) {
  ...
  X_test <- unemp_mbd[nrow(unemp_mbd), ]
  X_test$trend <- nrow(unemp_mbd)
  unemp_mbd <- unemp_mbd[-nrow(unemp_mbd),]
...
#Export ----------------------------------
write_rds(forest_forecast_ts, paste0(export,"other_cases/unemp/base_forecast.rds"))
write_rds(pred_ar1, paste0(export, "other_cases/unemp/ar1_forecast.rds"))