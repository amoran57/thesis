...
#Code ------------------------------------------
#Note that this is supposed to be a "bad" model. As such, it is not self-tuning
#A penalty term of 0.9 is imposed in conjunction with a minimum node size of 10
#Data is sampled randomly, and the feature fraction is 0.7
...
for (k in 1:253) {
  ...
  #fit the forest
  timestamp()
  tic(paste0("Forest iteration ", as.character(k), " complete"))
  ...
  n_rows <- lapply(bayes, function(x) answer <- nrow(x$pred))
  real_trees <- bayes[which(n_rows > 1)]
  n_real_trees <- length(real_trees)
  if(n_real_trees > 0) {
    real_preds_list <- lapply(real_trees, function(x) answer <- x$pred)
...
write_rds(forest_forecast_ts, paste0(export,"4_year_forecasts/mean_forecast_sample.rds"))
write_rds(all_mentions_df, paste0(export, "custom_forest_analysis/mean_forest_mentions.rds"))
