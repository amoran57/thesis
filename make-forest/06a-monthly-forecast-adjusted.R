...
get_adjustment <- function(penalty, formula, train_df, test_df, target, lag_name) {
...
bayesian_sprout_ar1_tree <- function(formula, feature_frac, sample_data = TRUE, minsize = NULL, data, penalties = NULL) {
  ...
  if(!is.null(penalties)) {
    ...
    adjustment <- get_adjustment(best_penalty, formula_new, train_df, test_df, target, lag_name = first_lag)
  ...
  # return the tree
  return(list(tree = bayes_tree, l_plot = l_plot, adjustment = adjustment))
}
...
bayes_reg_parallel_rf <- function(formula, n_trees = 50, feature_frac = 0.7, sample_data = TRUE, minsize = NULL, data, penalties = NULL) {
  ...
  if(n_trees <= split) {
    ...
    clusterExport(cl, c("libs", "formula", "n_trees", "feature_frac", "sample_data", 
                        ...
                        "split_lg", "get_rmses", "get_adjustment", "ar1_reg_tree"))
...
get_prediction <- function(forest, X_test) {
  ...
  for (i in 1:num_trees) {
    ...
    adjustment <- temp_tree$adjustment
    ...
    all_predictions[i] <- this_prediction/adjustment
  }
...
write_rds(forest_forecast_ts, paste0(export,"4_year_forecasts/ar1_obj_forecast_adjusted.rds"))
