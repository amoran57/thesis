...
ar1_sample_ts <- read_rds(paste0(export, "4_year_forecasts/ar1_obj_forecast_sample_straight.rds"))
...
graph_df$mean_diff <- graph_df$mean - graph_df$infl
...
graph_df$ar1_mean_closer <- ifelse(abs(graph_df$ar1_mean_diff) < abs(graph_df$arima_diff), "ar1", "arima")
graph_df$ar1_mean_closer <- as.factor(graph_df$ar1_mean_closer)
...
graph_df$mean_closer <- ifelse(abs(graph_df$mean_diff) < abs(graph_df$arima_diff), "mean", "arima")
...
mean_better <- graph_df %>% 
  filter(ar1_mean_closer == "ar1") %>% 
  select(date, arima, ar1_mean, infl, arima_diff, ar1_mean_diff)

arima_better <- graph_df %>% 
  filter(ar1_mean_closer == "arima") %>% 
  select(date, arima, ar1_mean, infl, arima_diff, ar1_mean_diff)
