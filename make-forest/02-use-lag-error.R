...
bayes_reg_parallel_rf <- function(formula, n_trees = 50, feature_frac = 0.7, sample_data = TRUE, minsize = NULL, data, penalties = NULL) {
  ...
  if(n_trees < split) {
    ...
    clusterExport(cl, c("x", "formula", "n_trees", "feature_frac", "sample_data", 
                        "minsize", "data", "penalties", "bayesian_sprout_tree_with_lag", "get_lag_errors", 
                        "evaluate_penalties", "generate_custom_random", "get_distribution",
                        "split_lg", "get_rmses", "predict_tree", "reg_tree", "sse_var"))
...
tic("Bayes RF")
bayes_forest_error <- bayes_reg_parallel_rf(formula, 50, feature_frac, sample_data, minsize, data, penalties)
toc()
tic("Grid RF")
