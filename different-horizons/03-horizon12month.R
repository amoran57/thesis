...
for (k in 1:length(monthly_dates)) {
  ...
  adj_y <- this_y[-c(1:11)]
  adj_x <- this_x[-c((nrow(this_x)-10):nrow(this_x)),]
...
#Predict using ARIMA -----------------------------
arima_monthly_dates <- seq(as.Date("1998/1/1"), as.Date("2019/1/1"), "month") 
pred_arima <- c()
...
#Export ----------------------------------
write_rds(forest_forecast_ts, paste0(export,"different_horizons/horizon12month/forecast.rds"))
write_rds(pred_arima, paste0(export, "different_horizons/horizon12month/arima_forecast.rds"))
