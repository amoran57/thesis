...
for (k in 1:length(monthly_dates)) {
  ...
  adj_y <- this_y[-c(1:2)]
  adj_x <- this_x[-c((nrow(this_x)-1):nrow(this_x)),]
...
#Predict using ARIMA -----------------------------
arima_monthly_dates <- seq(as.Date("1998/10/1"), as.Date("2019/10/1"), "month") 
pred_ar1 <- c()
for (k in 1:length(arima_monthly_dates)) {
  ...
  pred_a <- forecast(arima(train_tsData, order = c(1,0,0)), 3)$mean[3]
  
  pred_ar1[k] <- pred_a
}
pred_arima <- ts(pred_ar1, start = c(1999,1), frequency = 12)

naive <- window(tsData, start = c(1998, 10), end = c(2019, 10))
...
#Export ----------------------------------
write_rds(forest_forecast_ts, paste0(export,"different_horizons/horizon3month/base_forecast.rds"))
write_rds(pred_arima, paste0(export, "different_horizons/horizon3month/ar1_forecast.rds"))
write_rds(naive_forecast, paste0(export, "different_horizons/horizon3month/naive_forecast.rds"))
